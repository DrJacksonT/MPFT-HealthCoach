...
- Staff passwords use Argon2id and production TOTP secrets use authenticated encryption under a dedicated `MFA_ENCRYPTION_KEY`. Create accounts with the operator provisioning command in `account-setup.md`; do not seed shared staff credentials into production.
...
5. Keep `LIVE_PILOT_ENABLED=false`, `LIVE_AI_ENABLED=false` and `GAMBLING_PARTICIPANT_ENABLED=false` during smoke testing.
